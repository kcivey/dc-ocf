...
function getDcContributionsWithPositions(filters) {
    ...
    if (filters.committees) {
        query.whereIn('committee_name', filters.committees);
    }
    return query
...
function getContributorsByPlace(committeeName, year, byWard = false) {
...
function getCommitteesWithTopSharedContributors(committeeName, year, limit = 10) {
    return db.raw(
        ...
        WHERE c1.committee_name <> c2.committee_name
            AND c1.committee_name = ?
            AND c1.election_year = ?
        GROUP BY 1, 2, 3, 4
        ORDER BY 5 DESC, 1, 2, 3, 4
        LIMIT ?`,
        [committeeName, year, limit]
    );
}

function getContributorCount(committeeName, year) {
    return db(viewName)
