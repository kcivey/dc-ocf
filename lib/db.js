...
function getContributionInfo(filters, ward) {
    const query = db(`${contributionTableName} AS con`)
        .select(
            'office',
            'con.committee_name',
            'candidate_short_name',
            db.raw("SUM(CASE WHEN state = 'DC' THEN amount END) AS dc_amount"),
            db.raw("SUM(CASE WHEN contributor_type IN ('Individual', 'Candidate') " +
                'THEN amount END) AS ind_amount'),
            db.raw("SUM(CASE WHEN state = 'DC' AND contributor_type IN ('Individual', 'Candidate') " +
                'THEN amount END) AS dc_ind_amount'),
            db.raw('SUM(CASE WHEN amount > 0 THEN 1 WHEN amount < 0 THEN -1 END) AS contributions')
        )
        ...
        query
            .select(
                ...
                db.raw("SUM(CASE WHEN ward = ? AND contributor_type IN ('Individual', 'Candidate') " +
                    'THEN amount END) AS ward_ind_amount', [ward]),
            )
            .leftJoin(marTableName, 'number_and_street', 'ocf_address');
    }
    return query
        .join(`${committeeTableName} AS com`, 'con.committee_name', 'com.committee_name')
        .modify(addFilters, filters)
        .groupBy('office', 'con.committee_name', 'candidate_short_name')
        .orderBy('office')
