...
function getContributionInfo(filters, ward) {
    ...
    if (ward) {
        query
            ...
            .leftJoin(marTableName, 'contributions.number_and_street', 'mar.ocf_address');
    }
    return query.join('committees', 'contributions.committee_name', 'committees.committee_name')
        .modify(addFilters, filters)
        .groupBy('office', 'contributions.committee_name', 'candidate_short_name')
        .orderBy('office')
...
function getContributionSubtotals(filters, ward) {
    ...
    if (filters.office) {
        query.join('committees', 'contributions.committee_name', 'committees.committee_name');
    }
    return query.modify(addFilters, filters)
        .groupBy('contributions.committee_name', 'normalized', 'state')
        .having('subtotal', '>', 0);
}
