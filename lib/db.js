...
function getContributionInfo(filters, ward) {
    const query = db(viewName)
        .select(
            ...
            db.raw("SUM(CASE WHEN is_dc AND contributor_type = 'Individual' THEN " +
                'CASE WHEN amount > 50 THEN 50 ELSE amount END ELSE 0 END) AS fair_elections_eligible_amount'),
    ...
    if (ward) {
        query
            .select(
                ...
                db.raw(
                    'COUNT(DISTINCT CASE WHEN ward = ? AND is_individual THEN normalized END) AS ward_ind_contributors',
                    [ward]
                ),
            );
    }
...
async function getContributionStats({filters, bins, ward}) {
    ...
    for (const row of rows) {
        ...
        if (row.dc_ind_contributors > 150) { // @todo need to handle threshold and base amount for other offices
            row.fair_elections_addition = row.fair_elections_eligible_amount * 5 + 40000;
        }
        else {
            row.fair_elections_addition = 0;
        }
        row.fair_elections_total = row.amount + row.fair_elections_addition;
...
function getAvailableContests() {
    return db(viewName)
        .distinct('election_year', 'c.office')
...
function createCommitteeExtraRecords() {
    return db.raw(
        `INSERT OR IGNORE INTO committee_extras (committee_name, is_fair_elections, last_deadline)
        SELECT committee_name, FALSE, NULL FROM committees`
    );
}
...
function getCommittee(committeeName) {
    return db(committeeTableName)
        .select('*')
        .join(committeeExtraTableName, `${committeeTableName}.committee_name`,
