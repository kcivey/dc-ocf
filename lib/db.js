...
function createTables() {
    return db.schema.dropTableIfExists(contributionTableName)
        ...
        .then(() => db.raw(`DROP VIEW ${viewName}`))
        .then(function () {
            const groupColumns = [
                'com.election_year',
                'con.committee_name',
...
function getContributorsByPlace(candidateShortName, year, byWard = false) {
...
function getCommitteesWithTopSharedContributors(candidateShortName, year, limit = 10) {
    return db.raw(
        ...
        LIMIT ?`,
        [candidateShortName, year, limit]
    );
}

function getContributorCount(candidateShortName, year) {
    return db(viewName)
