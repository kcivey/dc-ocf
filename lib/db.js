        ...
        .then(function () {
            ...
            return db.raw(
                ...
                SELECT ${groupColumns},
                    ...
                    MAX(confidence_level) AS confidence_level,
...
function getContributionInfo(filters, ward) {
    const query = db(viewName)
        .select(
            ...
            'e.*',
            db.raw('MAX(e.is_fair_elections) AS is_fair_elections')
        )
...
async function getContributionStats({filters, bins}) {
    ...
    for (const row of rows) {
        ...
        if (row.is_fair_elections &&
            ...
            row.fair_elections_addition = row.fe_base_grant +
                Math.min(row.fair_elections_eligible_amount * 5, row.fe_max);
        }
        else {
...
function addFilters(query, filters) {
    if (filters) {
        ...
        if (filters.year) {
            const values = Array.isArray(filters.year) ? filters.year : [filters.year];
            query.andWhere('election_year', 'IN', values);
        }
...
function getLastDeadlines(office, year) {
    return db(committeeExtraTableName)
        ...
        .max('last_deadline AS last_deadline')
        .sum('amount AS subtotal')
...
function getLastContributionDates(office, year) {
    return db(committeeTableName)
        .select('candidate_short_name')
        ...
        .then(function (rows) {
            const data = {};
            for (const row of rows) {
                data[row.candidate_short_name] = row.receipt_date;
            }
            return data;
        });
}
...
function getAvailableContests(threshold, year, includePartyOffices = false) {
    return db(viewName)
        ...
        .then(function (rows) {
            ...
            let prevYear;
            for (const row of rows) {
                if (!includePartyOffices && /Committee|Party/.test(row.office)) {
                    continue;
                }
            ...
            for (const year of Object.keys(data)) {
                data[year].sort(function (a, b) {
                    return (officeRank(a) - officeRank(b)) || a.localeCompare(b);
                });
            }
            return data;
        });
...
async function setOcfLimits() {
    ...
    const ocfLimits = yaml.load(fs.readFileSync(yamlFile));
    const rows = await db(committeeExtraTableName)
        .select(`${committeeExtraTableName}.committee_name`, 'office', 'election_year')
        ...
        for (const [pattern, officeLimits] of Object.entries(ocfLimits)) {
            if (row.office.includes(pattern)) {
                ...
                for (const key of Object.keys(limits)) {
                    if (/^fe_/.test(key) && row.election_year < 2020) {
                        delete limits[key];
                    }
                    else if (typeof limits[key] !== 'number') {
                        limits[key] = limits[key][row.election_year];
                    }
                }
                break;
            }
...
async function getExcessContributions(committeeName) {
    const contributionLimit = await getContributionLimit(committeeName);
    return db.raw(
        `SELECT c.normalized,
            contributor_last_name AS last_name,
            contributor_first_name AS first_name,
            contributor_middle_name AS middle_name,
            number_and_street AS address,
            city, state, zip,
            receipt_date AS date,
            amount, total
        FROM contributions c
            INNER JOIN (
                SELECT normalized, SUM(amount) AS total
                FROM contributions
                WHERE committee_name = ?
                GROUP BY normalized
                HAVING total > ?
            ) e
            ON c.normalized = e.normalized
        WHERE committee_name = ?
        ORDER BY c.normalized, receipt_date, amount`,
        [committeeName, contributionLimit, committeeName]
    );
}

async function getContributionLimit(committeeName) {
    const c = await getCommittee(committeeName);
    return c.is_fair_elections ? c.fe_limit : c.traditional_limit;
}
...
module.exports = {
    ...
    getExcessContributions,
    getLastContributionDates,
    getLastDeadlines,
    getMatchingOffice,
    getOfficesForReport,
