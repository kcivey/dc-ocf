...
function parseAddress(address) {
    address = address.replace(/(?:WA|DC)-`?200/, 'Washington, DC 200'); // kluge for bad addresses
    address = address.replace(/( [A-Z][A-Z]-)\n(\d{5})/, '$1$2'); // another kluge
    address = address.replace(/(?:MA|NJ)-(\d{4})$/, 'MA-0$1'); // yet another kluge
